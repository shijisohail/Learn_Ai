...
│   ├── pillar_unit_economics_sketch.py       # Pillar deep dive: rough LLM monthly cost
│   ├── pillar_scheduled_agent_idempotency.py  # Pillar: idempotent event handler stub
│   ├── pillar_lead_score_baseline.py        # Pillar: toy lead scoring (tabular); --html → preview
│   └── pillar_lead_score_preview.html       # Generated by --html (animated metric + bars)
│
...
└── next/                   # "What's Next" deep-dive topic pages (beyond Day 30)
    ...
    ├── learning_pillars.html # Hub: map 3 pillars to days + next/ + stretch backlog
    ├── ai_product_building.html # Idea → PRD → economics → launch (AI product)
    ├── scheduled_agents.html # Cron, webhooks, idempotency, event-driven agents
    ├── tabular_ml_nlp_bridge.html # Tabular ML, entity resolution, NLP + LLM stack
    ├── experimentation_ab_testing.html # A/B testing, causal intuition, online vs offline
    ├── pillar-lesson.css       # Shared styles (day-style nav, hero, code, concept checks)
    └── pillar-interactive.js   # Shared UI: progress bar, reveals, hub filters, sliders, demos
```

## Features

- **30 interactive day lessons** with live widgets, code samples, and concept checks
- **16 "What's Next" deep-dive pages** (`next/`) covering A2A, MCP, Computer Use, Evals, Fine-tuning, Constitutional AI, learning pillars (product / agents / DS-ML), and more — for going beyond Day 30
...
## Learning Pillars Map

Three ways to orient the same curriculum — **AI product creation**, **AI agent development**, and **data science & machine learning**. Open [`next/learning_pillars.html`](next/learning_pillars.html) for a browseable hub with links.

### AI product creation

**Core days:** 6 (production), 9 (providers & cost), 20 (observability), 25 (auth/tenancy), 29–30 (architecture capstones).

**Deep dives:** [`next/ai_product_building.html`](next/ai_product_building.html)

**Stretch topics (backlog):** discovery interviews, pricing experiments, roadmap prioritization (e.g. RICE), GTM narrative for AI features, stakeholder alignment on quality vs cost.

### AI agent development

**Core days:** 2–5 (tools, RAG, LangGraph, multi-agent), 12 (advanced RAG), 13 (CrewAI).

**Deep dives:** [`next/evals.html`](next/evals.html), [`next/mcp.html`](next/mcp.html), [`next/a2a.html`](next/a2a.html), [`next/scheduled_agents.html`](next/scheduled_agents.html)

**Stretch topics:** human-in-the-loop review queues, multi-tenant agent isolation at scale, custom tool marketplaces, policy-as-code for tool use.

### Data science & machine learning

**Core days:** 15–21 (PyTorch, Hugging Face, LoRA, MLflow, MLOps, observability).

**Deep dives:** [`next/finetune.html`](next/finetune.html), [`next/tabular_ml_nlp_bridge.html`](next/tabular_ml_nlp_bridge.html), [`next/experimentation_ab_testing.html`](next/experimentation_ab_testing.html)

**Stretch topics:** large-scale entity resolution, uplift modeling, Bayesian A/B, joint drift detection for tabular + LLM features.

**Interview overlap:** See [`interview.html`](interview.html) (system design and production / MLOps sections) — complements these pillars without duplicating day lessons.
...
The `next/` folder contains 16 standalone topic pages for engineers who want to go deeper after completing the 30-day curriculum:

| Page | Topic |
|------|-------|
| `next/learning_pillars.html` | Hub: three pillars mapped to days + stretch backlog |
| `next/ai_product_building.html` | AI product: idea → PRD → economics → launch |
| `next/scheduled_agents.html` | Scheduled & event-driven agents (cron, webhooks, idempotency) |
| `next/tabular_ml_nlp_bridge.html` | Tabular ML, entity resolution, NLP pipelines + LLMs |
| `next/experimentation_ab_testing.html` | A/B testing, causal intuition, offline vs online metrics |
